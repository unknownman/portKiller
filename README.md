...
`lsof` is a wonderful firehose — but it dumps a wall of raw sockets at you. `killport` and `fkill` solve the "kill" part while dancing dangerously close to destroying whatever you were incubating in your dev server. **portkill** flips the flow: *look at the evidence, then decide.*
...
4. **Cross-platform honest** — on Windows, termination uses `TerminateProcess` semantics (no side-effect-free `SIGTERM` available); `--force` is the recommended path there. Crash-only signals are never mocked.
...
**"What about privileged ports (e.g. `80`)?"** — Inspecting is always possible. Killing a root-owned process may require appropriate privileges; portkill will report the exact reason if it can't.
